...
- **风险提示**：如果有不确定的地方，明说
- **初审问题 → 修复对照**（走过 auto-fix 时必附）：初审抓到哪些问题、每个问题的修复说明（`requesting-code-review` Step 7 的 issues log）

**验收引导清单（随交付附上——"过目"的最低标准，不是可选项）**：
- 🔴 改动（多文件/新模块/复杂算法）：请用户**实际打开 diff 核对关键文件**，不只看摘要——初审报告不能代替人眼核 diff
- 初审出过 failure 的：请用户**逐条看过修复说明**，确认"原问题 → 修法"对得上，而不是对着修复后的干净 diff 无从下嘴
- 风险提示：逐条确认是否接受
